...
class DCPDehazeGenerator(nn.Module):
    ...
    def __init__(self, win_size_a=5, win_size_t=15, eps=1e-3):
        super(DCPDehazeGenerator, self).__init__()

        self.guided_filter = GuidedFilter(r=win_size_t, eps=eps)
        self.win_size_a = win_size_a
    ...
    def forward(self, x):
        ...
        dark_img = self.get_dark_channel(imgPatch, self.win_size_a)
        map_A = self.atmospheric_light(imgPatch, dark_img)*2-1
         
        trans_raw = 1 - self.omega*self.get_dark_channel(imgPatch/((map_A + 1)/2), self.win_size_t)
